...
void DialogBoxFrame::render()
{
   if (!(al_is_system_installed()))
   {
      ...
      throw std::runtime_error("DialogBoxFrame::render: error: guard \"al_is_system_installed()\" not met");
   }
   if (!(al_is_primitives_addon_initialized()))
   {
      ...
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("DialogBoxFrame::render: error: guard \"al_is_primitives_addon_initialized()\" not met");
   }
