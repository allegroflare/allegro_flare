...
float ColumnWithLabels::render(bool only_calculate_height_dont_render)
{
   if (!(al_is_system_installed()))
   {
      ...
      throw std::runtime_error("ColumnWithLabels::render: error: guard \"al_is_system_installed()\" not met");
   }
   if (!(al_is_font_addon_initialized()))
   {
      ...
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("ColumnWithLabels::render: error: guard \"al_is_font_addon_initialized()\" not met");
   }
...
ALLEGRO_FONT* ColumnWithLabels::obtain_font()
{
   if (!(font_bin))
   {
      ...
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("ColumnWithLabels::obtain_font: error: guard \"font_bin\" not met");
   }
