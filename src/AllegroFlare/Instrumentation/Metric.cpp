...
void Metric::initialize()
{
   ...
   {
      std::stringstream error_message;
      error_message << "[Metric::initialize]: error: guard \"al_is_system_installed()\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("Metric::initialize: error: guard \"al_is_system_installed()\" not met");
   }
