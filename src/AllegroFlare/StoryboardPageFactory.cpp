...
AllegroFlare::Elements::StoryboardPages::Text* StoryboardPageFactory::create_text_page(std::string text)
{
   if (!(font_bin))
   {
      ...
      throw std::runtime_error("StoryboardPageFactory::create_text_page: error: guard \"font_bin\" not met");
   }
...
AllegroFlare::Elements::StoryboardPages::AdvancingText* StoryboardPageFactory::create_advancing_text_page(std::string text)
{
   if (!(font_bin))
   {
      ...
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("StoryboardPageFactory::create_advancing_text_page: error: guard \"font_bin\" not met");
   }
