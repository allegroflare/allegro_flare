...
void Layout::load_into_object(AllegroFlare::Tiled::TMJObject* object)
{
   if (!(object))
   {
      std::stringstream error_message;
      error_message << "[AllegroFlare::Layouts::Layout::load_into_object]: error: guard \"object\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AllegroFlare::Layouts::Layout::load_into_object]: error: guard \"object\" not met");
   }
   //
   // Load text objects
   //
...
void Layout::load_into_tilelayer(AllegroFlare::Tiled::TMJDataLoader* tmj_data_loader_ptr)
{
   if (!(tmj_data_loader_ptr))
   {
      std::stringstream error_message;
      error_message << "[AllegroFlare::Layouts::Layout::load_into_tilelayer]: error: guard \"tmj_data_loader_ptr\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AllegroFlare::Layouts::Layout::load_into_tilelayer]: error: guard \"tmj_data_loader_ptr\" not met");
   }
   auto &tmj_data_loader = *tmj_data_loader_ptr;
   //current_layer
   ...
   // Load the tile data
   //int num_rows = tmj_data_loader.get_num_rows(); // This data should be identical if it's in a layer or not
   //int num_columns = tmj_data_loader.get_num_columns(); // This data should be identical if it's in a layer or not
   ...
   if (current_group)
   {
      // Obtain lading data from the current group
      // NOTE: This should be symmetrical to the global level loader
      if (current_group->tilelayer_exists("visual")) visual_tilelayer_is_present = true;
      if (current_group->tilelayer_exists("opacity")) opacity_tilelayer_is_present = true;
   }
   else
   {
      // Obtain loading data from the global properties in the level loader
   ...
   if (visual_tilelayer_is_present)
   {
      ...
      //{
         ...
         //);
      //}
      std::vector<int> visual_tilelayer_data = current_group
                                             ? current_group->get_tilelayer_data_by_name("visual")
                                             : tmj_data_loader.get_tilelayer_data_by_name("visual");
      ...
      if (opacity_tilelayer_is_present)
      {
         opacity_tilelayer_data = current_group
                                ? current_group->get_tilelayer_data_by_name("opacity")
                                : tmj_data_loader.get_tilelayer_data_by_name("opacity");
      }
...
void Layout::set_first_found_text_slot_color(std::string name, ALLEGRO_COLOR color)
{
   // TODO: Test this
   AllegroFlare::Layouts::Elements::Text* text_slot = find_first_text_slot_or_throw(name);
