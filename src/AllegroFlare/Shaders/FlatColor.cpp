...
std::string FlatColor::obtain_fragment_source()
{
   static const std::string source = R"DELIM(
     ...
     vec4 color_it_plz(vec4 tmp)
     {
        ...
        tmp.r = tmp.r * inverse_color_intensity + color.r * color_intensity;
        tmp.g = tmp.g * inverse_color_intensity + color.g * color_intensity;
        tmp.b = tmp.b * inverse_color_intensity + color.b * color_intensity;
