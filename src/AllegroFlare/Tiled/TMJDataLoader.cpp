...
bool TMJDataLoader::load()
{
   ...
   current_loading_group = nullptr;
   extract_layers(&j, &tilesets_gids);
...
void TMJDataLoader::extract_layers(nlohmann::json* j_ptr, std::set<int>* tilesets_gids_ptr)
{
   if (!(j_ptr))
   {
      std::stringstream error_message;
      error_message << "[AllegroFlare::Tiled::TMJDataLoader::extract_layers]: error: guard \"j_ptr\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AllegroFlare::Tiled::TMJDataLoader::extract_layers]: error: guard \"j_ptr\" not met");
   }
   if (!(tilesets_gids_ptr))
   {
      std::stringstream error_message;
      error_message << "[AllegroFlare::Tiled::TMJDataLoader::extract_layers]: error: guard \"tilesets_gids_ptr\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AllegroFlare::Tiled::TMJDataLoader::extract_layers]: error: guard \"tilesets_gids_ptr\" not met");
   }
   auto &j = *j_ptr;
   ...
   for (auto &layer : j["layers"].items())
   {
      ...
      if (layer.value()["type"] == "group")
      {
         ...
         current_loading_group = &groups.back();

         // Recursively parse the layer
         extract_layers(&layer.value(), &tilesets_gids);

         current_loading_group = previous_group;
      }
      ...
      //
      // Load an "objectgroup" type layer
      //
