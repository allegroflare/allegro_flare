...
void TMJDataLoader::extract_layer(nlohmann::json* j_ptr, std::set<int>* tilesets_gids_ptr)
{
   if (!(j_ptr))
   {
      std::stringstream error_message;
      error_message << "[AllegroFlare::Tiled::TMJDataLoader::extract_layer]: error: guard \"j_ptr\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AllegroFlare::Tiled::TMJDataLoader::extract_layer]: error: guard \"j_ptr\" not met");
   }
   if (!(tilesets_gids_ptr))
   {
      std::stringstream error_message;
      error_message << "[AllegroFlare::Tiled::TMJDataLoader::extract_layer]: error: guard \"tilesets_gids_ptr\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AllegroFlare::Tiled::TMJDataLoader::extract_layer]: error: guard \"tilesets_gids_ptr\" not met");
   }
   auto &j = *j_ptr;
   auto &tilesets_gids = *tilesets_gids_ptr;
   // TODO: Move code into this method
