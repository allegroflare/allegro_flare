...
void PathObject::_recursive_traverse_dir(ALLEGRO_FS_ENTRY *folder)
{
...
int PathObject::_bfind(std::string filename)
{
...
// returns the full file path from the current directory
// this could be made more effecient with a binary search
std::string PathObject::find_from(std::string absolute_base_folder, std::string filename_to_find)
{
...
std::vector<std::string> PathObject::find_all_versions(std::string filename)
{
   ...
   {
      std::cout << "PathObject::find_latest(\"" << filename << "\") does not contain 2 ajacent # signs (##)" << std::endl;
      std::cout << "\tswitching to PathObject::find(\"" << filename << "\")" << std::endl;
      return matches;
