...
Inventory::Inventory(AllegroFlare::FontBin* font_bin, AllegroFlare::BitmapBin* bitmap_bin, AllegroFlare::Inventory* af_inventory, std::map<int, std::tuple<std::string, std::string, std::string>> inventory_index)
   : font_bin(font_bin)
...
void Inventory::set_inventory_index(std::map<int, std::tuple<std::string, std::string, std::string>> inventory_index)
{
...
void Inventory::scale_for(float width, float height)
{
   static int EXPECTED_STANDARD_WIDTH = 1920;
   static int EXPECTED_STANDARD_HEIGHT = 1080;

   place.position.x = width / 2;
   place.position.y = height / 2;
   place.scale.x = width / EXPECTED_STANDARD_WIDTH;
   place.scale.y = height / EXPECTED_STANDARD_HEIGHT;
...
ALLEGRO_COLOR Inventory::change_alpha(ALLEGRO_COLOR color, float alpha)
{
   color.a *= alpha;
   color.r *= color.a;
   color.g *= color.a;
...
{
   float o = 0.85f;
   ALLEGRO_COLOR full_window_fill = opaquify(change_alpha(al_color_html("11111d"), o));
   ALLEGRO_COLOR right_backfill_color = opaquify(change_alpha(al_color_html("11111d"), o));
   ALLEGRO_COLOR left_overbackfill_color = opaquify(change_alpha(al_color_html("16151c"), o));
   ALLEGRO_COLOR top_and_bottom_pin_line_color = opaquify(change_alpha(ALLEGRO_COLOR{0.6, 0.6, 0.6, 1.0}, o));

   // draw the overfill
   al_draw_filled_rounded_rectangle(-120, -80, place.size.x+120, place.size.y+80, 9, 9, full_window_fill);
