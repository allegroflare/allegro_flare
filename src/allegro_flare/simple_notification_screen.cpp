...
namespace allegro_flare
{
   ...
   void SimpleNotificationScreen::SimpleNotification::draw(float x_cursor, float y_cursor)
   {
      ...
      for (unsigned i=0; i<lines.size(); i++)
      {
         al_draw_text(font, AllegroFlare::color::color(text_color, AllegroFlare::clamp<float>(0, 1, lifespan)), x_cursor, y_cursor-text_height+al_get_font_line_height(font)*i, ALLEGRO_ALIGN_RIGHT, lines[i].c_str());
      }
   }
      ...
      for (unsigned i=0; i<notifications.size(); i++)
      {
         if (notifications[i].update())
         {
            al_draw_filled_rounded_rectangle(
                  ...
                  border_radius,
                  border_radius,
                  AllegroFlare::color::color(notifications[i].box_color, AllegroFlare::clamp<float>(0.0, 1.0, notifications[i].lifespan) * 0.2));
