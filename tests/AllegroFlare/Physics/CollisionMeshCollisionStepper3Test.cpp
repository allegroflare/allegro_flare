...
class AllegroFlare_Physics_CollisionMeshCollisionStepper3Test : public ::testing::Test {};
class AllegroFlare_Physics_CollisionMeshCollisionStepper3TestWithAllegroRenderingFixture
   : public AllegroFlare::Testing::WithAllegroRenderingFixture
{};
class AllegroFlare_Physics_CollisionMeshCollisionStepper3TestWithInteractionFixture
...
public:
   ...
   std::vector<std::pair<AllegroFlare::Placement3D*, AllegroFlare::Physics::CollisionMesh*>>
      collision_meshes_with_placements;
   ...
   AllegroFlare_Physics_CollisionMeshCollisionStepper3TestWithInteractionFixture()
      ...
      , hud_camera()
      , live_camera()
      , air_drag(AllegroFlare::Physics::CollisionMeshCollisionStepper3::DEFAULT_AIR_DRAG)
      , gravity(AllegroFlare::Physics::CollisionMeshCollisionStepper3::DEFAULT_GRAVITY)
   ...
   void setup_map(std::string map_model_identifier="collision_mesh_with_various_terrain_forms-05.obj")
   {
      ...
      auto collision_mesh = new AllegroFlare::Physics::CollisionMesh;
      collision_mesh->load(model_bin[map_model_identifier]);

      collision_meshes.push_back(collision_mesh);
      //collision_mesh->load();
   }
      ...
      while(interactive_test_wait_for_event())
      {
         ...
         switch(current_event.type)
         {
            case ALLEGRO_EVENT_TIMER:
            {
               ...
               AllegroFlare::Physics::CollisionMeshCollisionStepper3 stepper;
               ...
               stepper.set_collision_meshes(collision_meshes);
               stepper.set_collision_meshes_with_placements(collision_meshes_with_placements);
               //stepper.set_redirect_strategy(
                  //AllegroFlare::Physics::CollisionMeshCollisionStepper3::RedirectStrategy::REDIRECT_REFLECT_SURFACE
               //);
               stepper.step();
               ...
               for (auto &collision_mesh : collision_meshes) collision_mesh->draw(ALLEGRO_COLOR{0.2, 0.2, 0.3, 0.3});
               //AllegroFlare::Placement3D past_place_capture;
               //past_place_capture.start_transform();
               for (auto &collision_mesh_with_placement : collision_meshes_with_placements)
               {
                  if (collision_mesh_with_placement.first) collision_mesh_with_placement.first->start_transform();
                  collision_mesh_with_placement.second->draw(ALLEGRO_COLOR{0.2, 0.2, 0.3, 0.3});
                  if (collision_mesh_with_placement.first) collision_mesh_with_placement.first->restore_transform();
               }
               //past_place_capture.restore_transform();
               ...
               {
                  total_num_faces += collision_mesh->get_num_faces();
               }
               for (auto &collision_mesh_with_placement : collision_meshes_with_placements)
               {
                  total_num_faces += collision_mesh_with_placement.second->get_num_faces();
               }
...
TEST_F(AllegroFlare_Physics_CollisionMeshCollisionStepper3Test, can_be_created_without_blowing_up)
{
   AllegroFlare::Physics::CollisionMeshCollisionStepper3 collision_mesh3dcollision_stepper;
}
...
void create_response_diagram(
   ...
   AllegroFlare::Physics::CollisionMeshCollisionStepper3::StepoutStrategy stepout_strategy =
...
)
{
   ...
   AllegroFlare::Physics::CollisionMeshCollisionStepper3::move_to_time_of_collision_and_react_to_collision(
      ...
      stepout_strategy,
      redirect_strategy
      //AllegroFlare::Physics::CollisionMeshCollisionStepper3::StepoutStrategy::STEPOUT_DIRECTLY_UP,
      //AllegroFlare::Physics::CollisionMeshCollisionStepper3::StepoutStrategy::STEPOUT_REVERSE_VELOCITY,
      //AllegroFlare::Physics::CollisionMeshCollisionStepper3::STEPOUT_REFLECT_SURFACE
      //AllegroFlare::Physics::CollisionMeshCollisionStepper3::STEPOUT_EXTRUDE_FACE_NORMAL
      //AllegroFlare::Physics::CollisionMeshCollisionStepper3::RedirectStrategy::REDIRECT_DIRECTLY_UP
      //AllegroFlare::Physics::CollisionMeshCollisionStepper3::RedirectStrategy::REDIRECT_STOP_VERTICAL
      //AllegroFlare::Physics::CollisionMeshCollisionStepper3::RedirectStrategy::REDIRECT_SLIDE_ALONG_SURFACE
   );
   ...
   al_draw_multiline_textf(
      ...
      face_normal.z,
      face_collision_stepout,
      AllegroFlare::Physics::CollisionMeshCollisionStepper3::to_string(stepout_strategy).c_str(),
      AllegroFlare::Physics::CollisionMeshCollisionStepper3::to_string(redirect_strategy).c_str(),
...
TEST_F(AllegroFlare_Physics_CollisionMeshCollisionStepper3TestWithAllegroRenderingFixture,
...
occurs_on_a_single_entity)
{
   ...
   stepper.set__entities(&entities);
   //stepper.set_collision_mesh(collision_mesh);
   stepper.set_collision_meshes({ collision_mesh });
   ...
   CAPTURE__step__will_not_blow_up)
{
   ...
   for (int i=0; i<steps; i++)
   {
      ...
      //}
      for (auto &collision_mesh : stepper.get_collision_meshes())
      {
         collision_mesh->draw(ALLEGRO_COLOR{0.2, 0.2, 0.3, 0.3});
      }
...
TEST_F(AllegroFlare_Physics_CollisionMeshCollisionStepper3TestWithInteractionFixture,
...
{
   setup_map("multiple_collision_meshes-01-Scene-collision_mesh_main.obj");

   std::vector<std::string> collision_mesh_filenames = {
      "multiple_collision_meshes-01-Scene-collision_mesh_a.obj",
      "multiple_collision_meshes-01-Scene-collision_mesh_b.obj"
   };
   //std::vector<AllegroFlare::Physics::CollisionMesh*> collision_meshes;

   for (auto &collision_mesh_filename : collision_mesh_filenames)
   {
      auto result_collision_mesh = new AllegroFlare::Physics::CollisionMesh;
      result_collision_mesh->load(model_bin[collision_mesh_filename]);
      collision_meshes.push_back(result_collision_mesh);
   }

   // Run the simulation
   run_interactive_simulation();

   // Clean up the collision meshes
   for (auto &collision_mesh : collision_meshes) delete collision_mesh;
}


TEST_F(AllegroFlare_Physics_CollisionMeshCollisionStepper3TestWithInteractionFixture,
   INTERACTIVE__CAPTURE__when_meshes_have_placements__will_work_as_expected)
{
   setup_map("multiple_collision_meshes-01-Scene-collision_mesh_main.obj");

   AllegroFlare::Placement3D placement;

   placement.position.x = 1.0;
   placement.position.z = 3.0;
   auto &p = placement;
   placement.rotation.y = -0.25;

   std::vector<std::pair<AllegroFlare::Placement3D*, std::string>> collision_mesh_data = {
      { &p,      "multiple_collision_meshes-01-Scene-collision_mesh_a.obj" },
      { nullptr, "multiple_collision_meshes-01-Scene-collision_mesh_b.obj" },
   };
   //std::vector<AllegroFlare::Physics::CollisionMesh*> collision_meshes;

   for (auto &collision_mesh_datum : collision_mesh_data)
   {
      auto result_collision_mesh = new AllegroFlare::Physics::CollisionMesh;
      result_collision_mesh->load(model_bin[collision_mesh_datum.second]);
      collision_meshes_with_placements.push_back(std::make_pair(collision_mesh_datum.first, result_collision_mesh));
   }
   ...
   // Clean up the collision meshes
   for (auto &collision_mesh : collision_meshes_with_placements) delete collision_mesh.second;
}


TEST_F(AllegroFlare_Physics_CollisionMeshCollisionStepper3TestWithInteractionFixture,
   INTERACTIVE__CAPTURE__without_any_collision_mesh__will_work_as_expected)
{
   // A "passing" test would have the entity simply falling off the map (repeatedly, since it repositions back up
   // if it goes below the line in this test world).

   // Run the simulation
   run_interactive_simulation();

   // TODO: Have this test be non-interactive (expect velocity is applied to the subject, for example).
}
