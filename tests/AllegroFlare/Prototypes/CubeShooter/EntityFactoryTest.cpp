
#include <gtest/gtest.h>

#include <AllegroFlare/Prototypes/CubeShooter/EntityFactory.hpp>


TEST(AllegroFlare_Prototypes_CubeShooter_EntityFactoryTest, can_be_created_without_blowing_up)
{
   AllegroFlare::Prototypes::CubeShooter::EntityFactory entity_factory;
}



//#include <CubeShooter/Runner.hpp>

//TEST(AllegroFlare_Prototypes_CubeShooter_EntityFactoryTest, INTERACTIVE__run__returns_the_expected_response)
//{
   //AllegroFlare::Prototypes::CubeShooter::Runner runner;
   //runner.RUN("test");
//}


