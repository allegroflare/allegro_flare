...
CORE_ITEMS=appearance2d attr_save_load attributes automation bitmap_object blender camera2d camera3d color config cubemap data_attr decorated_cout display drawing_interface file_path file_path_object framework frustum generate_textures grid2d element_id image_processing md5 model motion render_music_notation object2d path2d placement2d placement3d profile_timer programming_language render_sample screen sha2 shader skeleton sound_object text_object timeline useful
...
bin/%$(BINARY_EXTENSION): tests/%.cpp lib/lib$(ALLEGROFLARE_LIB_NAME).a
	g++ -std=gnu++11 $< -o $@ -I$(ALLEGROFLARE_DIR)/include -I$(ALLEGRO_DIR)/include -L$(ALLEGROFLARE_DIR)/lib -l$(ALLEGROFLARE_LIB_NAME) -L$(ALLEGRO_LIB_DIR) $(ALLEGRO_TEST) -lboost_unit_test_framework
